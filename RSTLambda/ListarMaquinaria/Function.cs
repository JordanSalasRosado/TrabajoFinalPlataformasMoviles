...
public class Function
{
    ...
    public APIGatewayProxyResponse FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
    {
        try
        {
            ...
            {
                maquinarias = new MaquinariaRepository().ListarMaquinarias(filter);
            }
            else
            {
                maquinarias = new MaquinariaRepository().ListarMaquinarias();
            }

            return new APIGatewayProxyResponse
            {
                StatusCode = 200,
                Body = JsonConvert.SerializeObject(maquinarias),
